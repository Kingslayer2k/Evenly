...
function Chevron() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4 text-[#9CA3AF]" fill="none" stroke="currentColor" strokeWidth="2">
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[rgba(58,78,67,0.4)]" onClick={onClose}>
      <div
        role="dialog"
        aria-modal="true"
        className="mx-auto mt-3 w-[90%] max-w-[340px] overflow-hidden rounded-b-2xl bg-white shadow-[0_8px_24px_rgba(58,78,67,0.12)] transition duration-300 ease-out"
        ...
        <div className="bg-white px-0 py-2">
          ...
          >
            <span className="flex items-center gap-3">
              <MenuIcon type="gear" />
              <span className="text-[16px] font-normal text-[#1C1917]">Settings</span>
            </span>
            <Chevron />
          </button>
          ...
          <button
            type="button"
            className="flex h-[52px] w-full items-center justify-between px-4 text-left transition hover:bg-[#F7F7F5] active:bg-[#F3F4F6]"
            onClick={() => handleItemClick(onLogout || (() => console.log("Log out clicked")))}
          >
            <span className="flex items-center gap-3">
              <MenuIcon type="logout" />
              <span className="text-[16px] font-normal text-[#DC2626]">Log out</span>
            </span>
          </button>
        </div>
      </div>
