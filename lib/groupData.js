...
export async function createGroupWithMembership(supabase, user, profileName, name) {
  ...
  try {
    await ensureContextId(supabase, group.id, user.id);
  } catch (error) {
...
function buildContextPayloads(groupId, label, userId = null, type = "general", emoji = "🤝") {
  const basePayloads = [
    ...
    { group_id: groupId, title: label, emoji, closed: false, type },
    { group_id: groupId, title: label, emoji, closed: false },
    { group_id: groupId, name: label },
    { group_id: groupId, title: label },
  ];
...
export async function ensureContextId(supabase, groupId, userId = null) {
  ...
  const response = await insertWithFallbacks(
    supabase,
    "contexts",
    buildContextPayloads(groupId, "Shared", userId, "general", "🤝"),
    { select: "*" },
  );
...
export async function resolveContextId(supabase, groupId, contextId, contextName, userId = null) {
  ...
  if (normalizedName) {
    ...
    const response = await insertWithFallbacks(
      supabase,
      "contexts",
      buildContextPayloads(groupId, normalizedName, userId, "custom", "📍"),
      { select: "*" },
    );
