...
export async function loadUserGroupsBundle(supabase, user) {
  ...
  const [profileName, groupsResponse, membersResponse, expensesResponse, appearanceMap] = await Promise.all([
    ...
    supabase
      .from("groups")
      .select("id, name, join_code, code, created_at, card_color, color, card_image, background_image, image_url")
      .in("id", groupIds),
    supabase
      .from("group_members")
...
export async function loadGroupDetailBundle(supabase, groupId, user) {
  ...
  const [profileName, membershipResponse, groupResponse, membersResponse, expensesResponse, contextsResponse, settlementsResponse, appearanceMap] =
    await Promise.all([
      profileNamePromise,
