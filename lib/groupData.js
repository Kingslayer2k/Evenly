...
export async function createGroupWithMembership(supabase, user, profileName, name) {
  ...
  try {
    await ensureContextId(supabase, group.id);
  } catch (error) {
    console.error("Default context setup skipped:", error);
  }
...
export async function resolveContextId(supabase, groupId, contextId, contextName) {
  ...
  if (normalizedName) {
    ...
    const response = await insertWithFallbacks(
      ...
      "contexts",
      [
        { group_id: groupId, name: normalizedName, emoji: "📍", closed: false, type: "custom" },
        { group_id: groupId, name: normalizedName, emoji: "📍", closed: false },
        { group_id: groupId, title: normalizedName, emoji: "📍", closed: false },
        { group_id: groupId, name: normalizedName },
        { group_id: groupId, title: normalizedName },
      ],
      { select: "*" },
    );
  ...
  return ensureContextId(supabase, groupId);
}
...
export async function createExpenseRecord(supabase, user, payload) {
  ...
  try {
    resolvedContextId = await resolveContextId(supabase, groupId, contextId, contextName);
  } catch (error) {
    console.error("Context setup failed:", error);
    throw new Error("We couldn't set up a context for this expense yet.");
  }
