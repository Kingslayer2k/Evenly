...
const USER_GROUPS_CACHE_MS = 5 * 60 * 1000; // 5 min
const USER_CONTACTS_CACHE_MS = 5 * 60 * 1000; // 5 min
const GROUP_DETAIL_CACHE_MS = 2 * 60 * 1000; // 2 min
...
export async function createExpenseTemplate(supabase, {
  groupId,
  createdByMemberId,
  title,
  amountCents = 0,
  ...
  const now = new Date();
  // Schedule first auto-fire at start of next month on the chosen day
  const nextOccurrence = new Date(now.getFullYear(), now.getMonth() + 1, dayOfMonth);
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      split_method: splitMethod || "even",
      shares: shares || null,
      split_details: splitDetails || null,
      recurring_interval: recurringInterval,
      day_of_month: dayOfMonth,
      next_occurrence: nextOccurrenceStr,
      is_active: true,
    })
    .select("*")
    .single();
...
export async function loadExpenseTemplates(supabase, groupId) {
  const { data, error } = await supabase
    .from("expense_templates")
    .select("*")
    .eq("group_id", groupId)
  ...
  if (error) {
    const isTableMissing =
      String(error.code || "") === "42P01" ||
      /expense_templates/i.test(String(error.message || ""));
...
export async function deleteExpenseTemplate(supabase, templateId) {
...
// Settle net balance across multiple groups in one action.
// groupSettlements: Array<{ groupId, fromUserId, toUserId, amountCents }>
// Records one settlement per group, zeroing each group's A↔B balance.
export async function createNetSettlement(supabase, groupSettlements, paymentMethod = "other") {
  for (const item of groupSettlements) {
